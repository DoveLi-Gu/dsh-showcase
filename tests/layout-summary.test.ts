...
describe("generateLayoutSummary", () => {
  ...
  it("generates explicit English Markdown and poster output", async () => {
    ...
    expect(poster).toContain("font-size:clamp(1rem,1.05vw,1.25rem)");
    ...
    expect(poster).toContain('class="manifest"');
    expect(poster).toContain("grid-template-rows:auto minmax(.75rem,2.4vh) auto auto minmax(8rem,1fr) auto");
    expect(poster).toContain(".copy{position:relative;grid-row:1");
    expect(poster).toContain(".rail{position:relative;grid-row:3");
    expect(poster).toContain(".metrics{grid-row:4");
    expect(poster).toContain(".manifest{position:relative;grid-row:5");
    expect(poster).toContain(".footer{grid-row:6");
    expect(poster).not.toContain(".copy{position:absolute");
    expect(poster).not.toContain(".rail{position:absolute");
